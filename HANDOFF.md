...
> Kurz-Einstieg: Die **Messkampagne läuft** (EC2, unabhängig, 16+ Slots gelandet, Stop ~2026-06-23). Nur
...
- **Ziel:** 7 Tage = 56 Slots = 5.600/Endpunkt = 50.400 Calls. **Stop-Termin ~2026-06-23.** Stand 19.6.: 16+ Slots.
...
1. **Kernbeleg — LLM @ identischer Edge-RTT:** OpenAI/Groq/Mistral terminieren ALLE bei Cloudflare FRA. Seit 19.6.
   **für 100 % des LLM-Traffics gemessen + ASN-belegt** (je Host 2 CF-IPs ~50/50, alle AS13335 @ ~1 ms; Belege
   `data/audit_20260618/{l1_rtt_per_ip.md,asn_per_ip.md}`). `ttft` streut dennoch (Voll-Kampagne, 16 Slots,
   Zwischenstand): **~68 → 280 → 440 ms ≈ 6,5×** (groq<mistral<openai; finale Zahlen nach Kampagnenende).
   **Per-IP invariant**, Geografie sogar **invertiert** (EU-Mistral langsamer als US-Groq). HINWEIS (Audit H1):
   ...
   Kampagnen-Datensatz — daher die Voll-Kampagnen-Zahlen führen.
2. **2. Beleg:** Azure **schnellstes TTS** (`ttfa` ~94 ms, n=200). „Trotz US-Konkurrenz" gilt sauber nur gegenüber
   **Deepgram** (~280 ms connect, echter US-Transit). **OpenAI-TTS terminiert ebenfalls bei Cloudflare FRA**
   (gleiche IPs wie LLM-OpenAI, AS13335, ~6 ms connect) → zweite identical-edge-Instanz (~930 ms connect-exkl. =
   reines Backend) und **stärkt C1** (Audit M2).
...
> H1-Zahlen (Zwischenstand), H2-Eichungsreichweite, H4-L1-Quelle, ttl_ms; **noch in der Analyse zu beachten:**

- **H1 — Headline-Zahlen:** 75/268/476 ms = Predeploy-Pilot, nicht reproduzierbar; Voll-Kampagne ~68/280/440 ms
  (6,5×). In allen Docs auf Zwischenstand gesetzt; finale Zahlen nach Kampagnenende einsetzen.
...
- **H3 — Fehler-Filter:** Es gibt **kein** `error_kind`-Enum (F4 existiert nicht im Code). Filter `error=='timeout'`
  verfehlt 158/161 echte Timeouts. Roh-String per Teilstring bucketieren (158× `ReadTimeout`=tts_openai, 22× `http_503`,
  3× `timeout`) VOR jeder Failure-Gruppierung; Mapping dokumentieren.
...
> - **Fehler-/Timeout-Filter:** NICHT `error=='timeout'` (verfehlt 158/161). Roh-String per Teilstring (`ReadTimeout`) bucketieren.
> - **OpenAI-TTS-Fail-Rate real 10,4 %** (158/1519; Pre-Header-Hänger ~30 s; Audit M10). Latenz nur success-only.
> - **C1:** groq sauber disjunkt; **mistral<openai nur Median-Ordnung** (Tails überlappen) → Mann-Whitney/Bootstrap.
> - **Diurnal = Snapshot (16/56 Slots, Tag×Slot nicht-orthogonal)** — noch keine Tageszeit-Inferenz; volle 7×8-Kampagne
>   abwarten. **Deepgram-`ttfp` nach `ttfp_text` stratifizieren** (Mix „Good"/„Good morning" UTC-strukturiert, corr=0,915; Audit M9).
...
| `data/audit_20260618/ec2_data/` | Roh-Pull der Kampagne (16 Slots, L1/L2/Logs) — gitignored |
