...
Zielumfang: 7-Tage-Kampagne, 8 Zeitslots pro Tag, n=100 je Endpunkt und Slot, interleaved, insgesamt rund 50.000 Einzelmessungen. Ausgewertet werden alle 56 von 56 Slots der abgeschlossenen Vollkampagne; primär aggregiert über A4 (Median der Slot-Mediane), wobei die Aggregation (Median der Slot-Mediane gegenüber gepoolt) gegengeprüft wird und robuste Bootstrap-Konfidenzintervalle noch ausstehen. Die Latenz wird nur auf erfolgreichen Calls betrachtet (success-only), die Verfügbarkeit getrennt als eigene Achse ausgewiesen.

## Zentrales Ergebnis (Vollkampagne, 56 von 56 Slots, A4)

Die drei LLM-Anbieter terminieren **alle am selben Cloudflare-Edge in Frankfurt**: je Anbieter beide DNS-rotierten Edge-IPs als AS13335 bei ~1 ms TCP-RTT gemessen und per ASN-Lookup bestätigt (100 % des LLM-Traffics). Trotz nahezu identischer Netzanbindung streut die Antwortlatenz um **rund das Achtfache (~7,3× auf A4, gepoolt 8,3×)**, wobei die geografische Ordnung sogar invertiert ist: Ein EU-gehosteter Dienst ist langsamer als ein US-Dienst.
...
- **Stand:** Die Messinfrastruktur steht und ist mehrfach auditiert. Die Vollkampagne ist abgeschlossen (56/56 Slots), danach folgen Auswertung, Statistik und schriftliche Ausarbeitung (geplante Fertigstellung voraussichtlich im August 2026).
